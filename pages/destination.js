...
class Destination extends React.PureComponent {
  ...
  static async getInitialProps() {
    const posts = await api.posts().order('asc').orderby('title').perPage(30).category(5);
    return { posts };
  }
  ...
  loadMore = async () => {
    ...
    const posts = await api.posts().order('asc').orderby('title').perPage(30).category(5).page(this.state.page + 1);
    if (posts.length > 0) {
      this.setState({
  ...
  getTotalPages = async () => {
    await api.posts().order('asc').orderby('title').perPage(30).category(5).then((response) => {
      this.setState({
