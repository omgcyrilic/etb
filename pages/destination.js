...
class Destination extends React.PureComponent {
  ...
  static async getInitialProps() {
    const posts = await api.posts().order('asc').orderby('title').perPage(10).category(5).embed();
    return { posts };
  }
  ...
  loadMore = async () => {
    ...
    const posts = await api.posts().order('asc').orderby('title').perPage(10).category(5).page(this.state.page + 1).embed();
    if (posts.length > 0) {
      this.setState({
